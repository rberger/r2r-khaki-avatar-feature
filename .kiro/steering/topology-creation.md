...
> **Note**: The Tables entity is not yet available in tc-functors. DynamoDB tables must be created via AWS SDK scripts (e.g., `scripts/create-table.py`).

```yaml
# NOT YET SUPPORTED - use AWS SDK script instead
# tables:
#   table-name:
...
- **Tables entity not available**: DynamoDB tables must be created via AWS SDK scripts, not in topology.yml. See `scripts/create-table.py` for provisioning.
...
## Getting tc Documentation

When you need tc-functors documentation or have questions about syntax/patterns:

1. Use the DeepWiki MCP tool to query the `tc-functors/tc` repository:
   - Tool: `mcp_deepwiki_ask_question`
   - Parameter: `repoName: "tc-functors/tc"`
   - Example: `mcp_deepwiki_ask_question(repoName: "tc-functors/tc", question: "How do I configure Cognito authentication?")`

2. Reference documentation:
   - Full docs: <https://tc-functors.org>
   - Specification: <https://tc-functors.org/reference/specification/>
   - Examples: <https://github.com/tc-functors/tc/tree/main/examples>
